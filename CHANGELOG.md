...
- **The permission system replaces `AccessType`.** The auth plugin is now `{ authenticate, authorize }`: `authenticate(req)` is the old `readAuthInfo` renamed with one semantic change — **`null` is an anonymous caller, not a rejection**: `authorize` is then asked with `user: null` and may grant (public documents); reject a presented credential with a branded `apiError(401, ...)` — and `authorize(scope, resourceId, user)` replaces `getAccessType`/`getOrgAccessType`/`getGlobalAccessType`, answering one of four typed permission objects per scope — `'permissions:document:v1'` with the full facet vocabulary (`ydoc`/`awareness` positional CRUD masks, a `history` from-ray carrying `rollback`/`prune`, `delete` kinds, and an `endpoint` map with a `'*'` fallback) and endpoint-only `branch`/`org`/`global` objects. The return type is forced to match the scope — implement via `createAuthorize({ document: async (docRef, user) => ... })` (one fully-typechecked handler per scope, unlisted scopes deny; a hand-rolled function needs a cast) — and every answer is validated against its scope's permission schema at runtime (an invalid or wrong-scope object is a loud `500`, never a silent denial). `null` denies — and stays `null`: `req.permissions` and a connection's view are nullable, nothing fabricates a permission object, and `hasPermissions(null, ..)` is false (the ordinary `missing-permission` 403). A throw from either hook is an infrastructure failure (REST and websocket upgrade `503`, websocket re-check `1013`; a branded `apiError` passes its status through). yhub never answers `401` for a *missing* credential — the one built-in exception is that **writing the document needs an identity** (attributions carry the userid): an anonymous caller holding ydoc `u` gets `401 { code: 'unauthenticated' }` from `PATCH /ydoc` (`update`), `POST /rollback`, and at the websocket upgrade, always after the permission check; reads, presence, history, prune and delete (`by: null`) work anonymously when granted. Deleted: `$accessType`, `hasReadAccess`/`hasWriteAccess`, `accessPurpose`, `req.accessType` (now `req.permissions`). Migrate `'r'|'rw'|null` answers with the documented mapping ([API docs](API.md#migrating-from-accesstype)) — it deliberately excludes `rollback`, `prune`, and `delete`: destructive rights are granted by name, never implied by a write mask. Behavior changes: read-only connections granted awareness `u` now broadcast presence (the old blanket write gate swallowed their cursors); `DELETE /ydoc` requires the `delete` facet and `?hard=true` becomes possible when `'hard'` is granted (previously programmatic-only); `gc=false` requires full history access; changeset/activity clamp `from` to the granted ray and their `?ydoc=`/`?delta=` flags require ydoc read; every rest endpoint — builtin and custom — is gated by the `endpoint` facet (mask position by HTTP verb) before its handler runs, while handlers state the semantic facets they touch as one permission fragment via `checkPermissions(req.permissions, createDocumentPermissions({ ydoc: '-r--', ... }))` (both exported from `@y/hub`, with `createOrgPermissions`/`createGlobalPermissions`/`createBranchPermissions`/`createPermissions(scope, facets)` — prototype-free input-form objects with the `type` filled in; built on the containment check `hasPermissions(granted, required)` over the *normalized* view (or `null`) — `required ⊆ granted`, decided as intersect-then-compare in the merge algebra, where requiring `rollback`/`prune` implies requiring ydoc `u`, the fragment's allowed facets are typed to the scope (a doc facet against an org permission is a compile error), and a malformed fragment — an unknown facet at any depth, an invalid value, or a pure denial — throws instead of silently weakening the check); registration refuses custom endpoints reusing a built-in name in any version (withdraws the documented `name: 'ydoc', version: 'v2'` pattern); permission `403`s answer `{ error: 'requires permission {...}', code: 'missing-permission', required }` with the request's whole requirement as the fragment (changeset/activity limit the query's `from` to the granted ray first and require exactly that `history: { from }` — reads clamp, never refuse; changeset/activity/rollback/prune validate `from`/`to` as non-negative integers — `400` otherwise — and the reads lift `to` to the clamped `from`). The websocket route is the endpoint named `ws` in the `endpoint` facet: the upgrade needs `endpoint.ws` `r` next to ydoc `r`, doc updates over the socket need `endpoint.ws` `u` next to ydoc `u` — `'*'` covers it, so grants with a fallback are unaffected, while a grant without any `endpoint` entry no longer opens a socket; the websocket re-check compares the effective `ws` mask too (presence broadcasts stay on awareness `u` alone). The permissions module ships as `@y/hub/permissions` (schemas, the creators, `sanitizePermissions`, merge algebra, `hasPermissions` over normalized views, normalizers). Spelling: the tier is `document` everywhere — custom endpoints' `scope: 'doc'` is now `scope: 'document'` (the default), the types and helpers are `DocumentPermissionsV1`, `$documentPermissionsV1`, `normalizeDocumentPermissions`, `documentPermissionsUnion`/`Intersect`, `ApiDocumentRequest`; the schema literal stays `'permissions:document:v1'`, `docid`/`docRef` stay. ([proposal](proposals/permissions.md), [API docs](API.md#permissions), [`src/permissions.js`](src/permissions.js), [`src/api.js`](src/api.js), [`src/server.js`](src/server.js), [`src/builtin-api.js`](src/builtin-api.js))
- **"Room" is retired: the `{org, docid, branch}` triple is a `DocRef`** (see [proposals/naming.md](proposals/naming.md)). `$room` → `$docRef`, the `Room` typedef → `DocRef`, `reqToRoom` → `reqToDocRef`, `req.room` → `req.docRef`, worker-event payloads' `room` → `docRef`, `DocDeletedError.room` → `.docRef`, log fields. Unchanged: `docid`/`branch` params, routes, SQL columns, `deleteDoc`, and every serialized redis spelling (`{prefix}:room:…` stream keys, `quarantine_room:…`) — the data-plane rename is deferred until it ships with a drain/dual-read migration. ([`src/types.js`](src/types.js))
- **`demos/` and `bin/auth-server-example.js` are removed.** They demonstrated the retired y-redis-era external-auth flow (`GET /auth/perm/:room/:userid` answering `yaccess` values) that yhub itself never calls. The `AUTH_PUBLIC_KEY`/`AUTH_PRIVATE_KEY` entries left `.env.template` with them — nothing in yhub reads those variables; JWT signing keys belong to your application (examples in [GETTING-STARTED.md](GETTING-STARTED.md)).
