...
export const testAgentUpdatesPropagate = async tc => {
  ...
  await yhub.agentTask(defaultDocRef, { author: 'claude' }, async (ydoc) => {
...
export const testDisplayedAuthor = async tc => {
  ...
  await yhub.agentTask(defaultDocRef, { author: 'claude', clearAwareness: false }, async (_ydoc, aw) => {
  ...
  let agentCid2 = 0
  await yhub.agentTask(defaultDocRef, { author: 'claude', displayedAuthor: 'Claude', clearAwareness: false }, async (_ydoc, aw) => {
...
export const testClearAwarenessImmediate = async tc => {
  ...
  const { provider } = await createWsClient({ waitForSync: true })
  let agentCid = 0
  await yhub.agentTask(defaultDocRef, { author: 'claude' }, async (_ydoc, aw) => {
    agentCid = aw.clientID
...
export const testClearAwarenessDelayed = async tc => {
  ...
  const task = yhub.agentTask(defaultDocRef, { author: 'claude', clearAwareness: 0.4 }, async (_ydoc, aw) => {
    agentCid = aw.clientID
    await waitFor(() => provider.awareness.states.has(agentCid))
...
export const testErrorClearsImmediatelyAndRejects = async tc => {
  ...
  let caught = null
  try {
    await yhub.agentTask(defaultDocRef, { author: 'claude', clearAwareness: 5 }, async (_ydoc, aw) => {
      agentCid = aw.clientID
      await waitFor(() => provider.awareness.states.has(agentCid))
...
export const testAttributionRecorded = async tc => {
  ...
  await yhub.agentTask(defaultDocRef, {
    author: 'claude-id-42',
  ...
  const rendered = verify.get().toDelta({ renderer: new Y.AttributionsRenderer({ inserts: Y.diffIdMap(attributions.inserts, attributions.deletes), deletes: Y.createIdMap() }) })
  ...
  // `insert` is multi-valued (array) by Y.AttributionsRenderer convention;
...
/**
 * The ydoc passed to the handler is hydrated from the document's current state.
 *
 * @param {t.TestCase} tc
 */
