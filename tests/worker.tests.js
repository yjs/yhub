 ...
 * `taskConcurrency: 500` on `yhub:testing`, so it would claim any task we park - and a room
 ...
 * @param {(event: { room: import('../src/types.js').Room, timestamp: number }) => void} [conf.taskStart]
 * @param {(event: { room: import('../src/types.js').Room, duration: number, error: Error|null }) => void} [conf.taskComplete]
 * @param {string} prefix
 */
 ...
 */
const seedRoom = (hub, room, content) => {
  ...
  return hub.stream.addMessage(room, { type: 'ydoc:update:v1', update, contentmap })
}

/**
 * Replace `computePool.mergeUpdates` so that compacting `room` blocks. Only the gc'd merge is
 ...
 */
const blockCompaction = (hub, room, blocked) => {
  ...
  hub.computePool.mergeUpdates = async (gc, updates, logContext = {}, prune) => {
    if (gc && logContext.room?.docid === room.docid) await blocked
 ...
 * worker must keep claiming and completing tasks for other rooms.
 *
 * @param {t.TestCase} tc
 */
export const testTaskLeaseSurvivesLongCompute = async tc => {
  ...
  const hub = await createWorkerHub({
    taskDebounce,
    taskStart: ({ room }) => started.push(room.docid),
    taskComplete: ({ room }) => completed.push(room.docid)
  }, prefix)
  const slowRoom = { org: utils.defaultOrg, docid: tc.testName + '-slow', branch: 'main' }
  const fastRoom = { org: utils.defaultOrg, docid: tc.testName + '-fast', branch: 'main' }
  /**
   * @type {() => void}
   */
  let unblock = () => {}
  blockCompaction(hub, slowRoom, promise.create(resolve => { unblock = () => resolve(undefined) }))
  /**
   * @type {Array<string>}
   */
  const stored = []
  const store = hub.persistence.store.bind(hub.persistence)
  hub.persistence.store = (room, doc) => { stored.push(room.docid); return store(room, doc) }

  await seedRoom(hub, slowRoom, 'slow')
  const slowTaskId = /** @type {string} */ ((await utils.yhub.stream.redis.xRange(hub.stream.workerStreamName, '-', '+')).find(e => e.message.compact === stream.encodeRoomName(slowRoom, prefix))?.id)
  t.assert(slowTaskId != null, 'seeding enqueued a compact task for the slow room')
  ...
  // the claim loop kept working while the slow room was blocked
  await seedRoom(hub, fastRoom, 'fast')
  await promise.untilAsync(() => completed.includes(fastRoom.docid), 10000, 50)
  t.assert(!completed.includes(slowRoom.docid), 'the slow room is still being compacted')

  unblock()
  await waitDrained(hub)
  t.compare(started.filter(docid => docid === slowRoom.docid).length, 1, 'the slow room was compacted exactly once')
  t.compare(stored.filter(docid => docid === slowRoom.docid).length, 1, 'the slow room was persisted exactly once')
  const { gcDoc } = await hub.persistence.retrieveDoc(slowRoom, { gc: true })
  const restored = new Y.Doc()
  gcDoc.forEach(update => Y.applyUpdate(restored, update))
  t.compare(restored.get('text').toString(), 'slow', 'the slow room was persisted correctly')
  hub.stopWorker()
}
 ...
 * another worker picks it up: lease renewal must never make a room permanently unreclaimable.
 *
 * @param {t.TestCase} tc
 */
export const testHangingTaskIsAbandonedAndReclaimed = async tc => {
   ...
   */
  const startedA = []
  const hubA = await createWorkerHub({ taskDebounce, maxTaskDuration: 500, taskStart: ({ room }) => startedA.push(room.docid) }, prefix)
  const room = { org: utils.defaultOrg, docid: tc.testName + '-index', branch: 'main' }
  ...
  blockCompaction(hubA, room, promise.create(() => {}))
  /**
   * @type {Array<string>}
   */
  const storedA = []
  ...
  await seedRoom(hubA, room, 'stuck')
  ...
  // a real deployment keeps hubA around and it would keep retrying the room. Take it out here so
  ...
  const { gcDoc } = await hubB.persistence.retrieveDoc(room, { gc: true })
  const restored = new Y.Doc()
  gcDoc.forEach(update => Y.applyUpdate(restored, update))
  t.compare(restored.get('text').toString(), 'stuck', 'the reclaimed room was compacted by the second worker')
  hubB.stopWorker()
}

/**
 * Two workers compacting the same room is expected and must be harmless. The dangerous
...
export const testConcurrentCompactionKeepsTheDocument = async tc => {
  ...
  hubB.stopWorker()
  const room = { org: utils.defaultOrg, docid: tc.testName + '-index', branch: 'main' }
  await seedRoom(hubA, room, 'important content')
  const entry = (await utils.yhub.stream.redis.xRange(hubA.stream.workerStreamName, '-', '+'))
    .find(e => e.message.compact === stream.encodeRoomName(room, prefix))
  const task = /** @type {any} */ ({ type: 'compact', room, redisClock: /** @type {any} */ (entry).id })
  ...
  // A compacts and persists the whole room while B is stalled
  await hubA._runTask(task)
  release()
  await bDone

  const { gcDoc } = await hubA.persistence.retrieveDoc(room, { gc: true })
  t.assert(gcDoc.length > 0, 'the room is still persisted after both workers finished')
  const restored = new Y.Doc()
  gcDoc.forEach(update => Y.applyUpdate(restored, update))
