...
export const testSyncAndCleanup = async tc => {
  ...
  const { ydoc: doc1 } = createWsClient()
  // doc2: can retrieve changes propagated on stream
  const { ydoc: doc2 } = createWsClient()
  ...
  const { ydoc: doc3 } = createWsClient()
  ...
  const { ydoc: doc4 } = createWsClient()
  await utils.waitDocsSynced(doc3, doc4)
  t.info('docs synced (3)')
  t.assert(doc3.get().getAttr('a') === 1)
  const { references } = await yhub.getDoc(defaultRoom, { references: true })
  ...
  const { references: references2 } = await yhub.getDoc(defaultRoom, { references: true })
